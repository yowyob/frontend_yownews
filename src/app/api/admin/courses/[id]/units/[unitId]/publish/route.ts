...
// POST /api/admin/courses/{id}/units/{unitId}/publish — surcharge le statut d'une unité,
// indépendamment du cours parent (l'héritage automatique reste le comportement par défaut).
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string; unitId: string }> }) {
  return handleRoute(async () => {
    ...
    return educationApi.publishCourseUnit(session, id, unitId);
  });
}
