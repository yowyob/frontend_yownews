...
// Streame l'image de couverture depuis KSM (GET /api/v1/education/blogs/{id}/coverblog).
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const res = await educationApi.getBlogCover(session, id);
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return handleRoute(async () => {
    ...
    return educationApi.uploadBlogCover(session, id, forward);
  });
}
